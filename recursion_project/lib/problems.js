...
function pow(base, exponent) {
    if (exponent === 0) {
        ...
        return 1/(base * pow(base, -1-exponent));
    }
}
...
function flatten(data) {
    if (!Array.isArray(data)) {
        ...
        let flat = [];
        data.forEach((datum) => {
            flat = flat.concat(flatten(datum));
        })
        return flat;
    }
}
...
function fileFinder(directories, targetFile) {
    let top = Object.keys(directories);
    let res = false;
    if ( top.includes(targetFile) ) {
        return true;
    } else {
        top.forEach((key) => {
...
function pathFinder(directories, targetFile) {
    ...
    if (top.includes(targetFile)) {
        ...
        top.forEach((key) => {
            if (directories[key] !== null) {
                let secpath = pathFinder(directories[key], targetFile);
                if (!!secpath) {
                    path = key;
                    path += secpath;
                };
